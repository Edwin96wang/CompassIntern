void Recoil_proton_comp(){
    ...
    file2->GetObject("angle_proton", graph_data2);

    //data editting
    double max = graph_data->GetEntries();
    for(int i = 1; i <= graph_data->GetNbinsX(); i++){
        graph_data->SetBinContent(i, 1000*graph_data->GetBinContent(i)/max);
    }
    max = graph_data2->GetEntries();
    ...
    graph_data->GetXaxis()->SetRangeUser(0,3);
